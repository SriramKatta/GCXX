...
TEST_F(EventWrapTest, RecordInStreamWithFlags) {
  ...
  // EXPECT_NO_THROW(event.RecordInStream(test_stream, flags::eventRecord::external)); //
}
...
TEST_F(EventWrapTest, ElapsedTimeBetweenStatic) {
  ...
  auto elapsed_nano = event_wrap::ElapsedTimeBetween<nanoSec>(start_event, end_event);
...
TEST_F(EventWrapTest, DurationTypeAliases) {
  ...
  EXPECT_TRUE((std::is_same_v<nanoSec, std::chrono::duration<float, std::nano>>));
  EXPECT_TRUE((std::is_same_v<microSec, std::chrono::duration<float, std::micro>>));
  EXPECT_TRUE((std::is_same_v<milliSec, std::chrono::duration<float, std::milli>>));
