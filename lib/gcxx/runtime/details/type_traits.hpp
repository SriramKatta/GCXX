...
GCXX_DIAG_SUPPRESS_ALL_EXEC_CHECKS_()
...
GCXX_DIAG_RESTORE_ALL_EXEC_CHECKS_()
...
GCXX_FH auto get_raw_pointer(VT&& ptr) -> decltype(ptr.get()) {
  // Forwarding keeps the check honest even though only .get() is used.
  return std::forward<VT>(ptr).get();
}
