...
// build rank 1 mdspan for use with Level 1&2 blas using a linear data structure
// like vector, buffer or spans
//
...
constexpr auto make_vector(SpanLike&& storage, Idx stride = Idx{1})
  ...
  // length = number of strided elements that fit in the storage (stride >= 1)
  const Idx cap = static_cast<Idx>(gcxx::details_::size(storage));
  const Idx n   = (cap > Idx{0}) ? ((cap - Idx{1}) / stride) + Idx{1} : Idx{0};
