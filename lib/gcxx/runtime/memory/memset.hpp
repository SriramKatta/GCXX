...
namespace memory {
  ...
  GCXX_FH()
  auto Memset(VT* dev_ptr, const int value,
  ...
  template <typename VT>
  GCXX_FH()
  auto Memset(const VT* dev_ptr, const int value, const std::size_t numElements,
              const StreamView& stream) -> void {
  ...
  template <typename VT>
  GCXX_FH()
  auto Memset(device_ptr<VT>& destination, const int value,
              const std::size_t numElements) -> void {
  ...
  GCXX_FH()
  auto Memset(device_ptr<VT>& destination, const int value,
              const std::size_t numElements, const StreamView& stream) -> void {
    details_::Memset(destination.get(), value, numElements * sizeof(VT),
                     stream);
  }
  ...
  GCXX_FH()
  auto Memset(Ptr& handle, const int value,
              const std::size_t numElements) -> void {
  ...
  template <typename Ptr,
            typename = std::void_t<decltype(std::declval<Ptr&>().get())>>
  GCXX_FH()
  auto Memset(Ptr& handle, const int value, const std::size_t numElements,
              const StreamView& stream) -> void {
    using raw_ptr_t = decltype(std::declval<Ptr&>().get());
    using VT        = std::remove_pointer_t<std::remove_cv_t<raw_ptr_t>>;
  ...
  template <typename VT>
  GCXX_FH()
  auto Memset(span<VT> destination, const int value) -> void {
    details_::Memset(destination.data(), value, destination.size_bytes());
  }

  template <typename VT>
  GCXX_FH()
  auto Memset(const span<VT> destination, const int value,
              const StreamView& stream) -> void {
    details_::Memset(destination.data(), value, destination.size_bytes(),
                     stream);
  }
