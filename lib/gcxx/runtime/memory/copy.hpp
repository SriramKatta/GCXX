...
GCXX_FH auto copy(void* desstination, const void* source,
  ...
  GCXX_SAFE_RUNTIME_CALL(Memcpy, "Failed to perform async GPU copy", desstination, source,
                         countinBytes, GCXX_RUNTIME_BACKEND(MemcpyDefault));
}

GCXX_FH auto copy(void* desstination, const void* source, const std::size_t countinBytes,
                  const stream_wrap& stream) -> void {
  GCXX_SAFE_RUNTIME_CALL(MemcpyAsync, "Failed to perform async GPU copy", desstination,
                         source, countinBytes, GCXX_RUNTIME_BACKEND(MemcpyDefault),
                         stream.get());
}
...
namespace memory {
  ...
  GCXX_FH auto copy(VT* desstination, const VT* source,
    ...
    details_::copy(static_cast<void*>(desstination), static_cast<const void*>(source),
                   numElements * sizeof(VT));
  }

  template <typename VT>
  GCXX_FH auto copy(const VT* desstination, const VT* source, const std::size_t numElements,
                    const stream_wrap& stream) -> void {
    details_::copy(static_cast<void*>(desstination), static_cast<const void*>(source),
