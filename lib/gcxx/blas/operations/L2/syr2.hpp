...
#pragma once
#ifndef GCXX_BLAS_OPERATIONS_L2_SYR2_HPP_
#define GCXX_BLAS_OPERATIONS_L2_SYR2_HPP_
...
// A += alpha*(x*y^T + y*x^T) into the tagged triangle; scaled() alpha.
...
auto symmetric_matrix_rank_2_update(
  ...
  static_assert(!gcxx::is_scaled_accessor_v<AccessorA>,
                "symmetric_matrix_rank_2_update outputs cannot be scaled() "
                "views; scale x or y instead");

  static_assert(gcxx::details_::all_same_v<XIt, YIt, AIt>,
                "symmetric_matrix_rank_2_update operands x, y, A must share "
  ...
  static_assert(gcxx::blas::details_::is_supported_blas_element_v<AVt>,
                "symmetric_matrix_rank_2_update currently supports only "
  ...
  if (rows_a != cols_a) {
    details_::throwBlasError(GCXX_BLAS_STATUS(INVALID_VALUE),
                             "symmetric_matrix_rank_2_update requires A to "
                             "be square");
  }
  if (len_x != rows_a || len_y != rows_a) {
    details_::throwBlasError(GCXX_BLAS_STATUS(INVALID_VALUE),
                             "symmetric_matrix_rank_2_update requires x and "
  ...
  GCXX_BLAS_DISPATCH_TYPED(status, XIt, XVt, syr2, h.getRawHandle(), uplo,
                           rows_a, alpha_ptr, x.data_handle(), inc_x,
                           y.data_handle(), inc_y, a.data_handle(), ld_a);

  if (status != driver::deviceBlasStatusSuccess) {
    details_::throwBlasError(status, "symmetric_matrix_rank_2_update failed");
  }
}
