...
#pragma once
#ifndef GCXX_BLAS_OPERATIONS_L1_IAMAX_HPP_
#define GCXX_BLAS_OPERATIONS_L1_IAMAX_HPP_
...
// Zero-based index of max |x[i]|; backend is one-based, translated here.
...
auto idx_abs_max(BlasHandleView h,
  ...
  if constexpr (std::is_same_v<XVt, gcxx::f32_t>) {
    ...
    GCXX_BLAS_DISPATCH_INT64(status, XIt, Idamax, h.getRawHandle(), len_x,
                             x.data_handle(), inc_x, &result);
  }
