...
### [cubDeviceSegmentedScan](./cubDeviceSegmentedScan) [ ]
This sample demonstrates `cub::DeviceSegmentedScan`. A segmented scan computes an independent scan over each of many contiguous segments in a single device-wide call.

### [cubDeviceTransform](./cubDeviceTransform) [ ]
This sample demonstrates `cub::DeviceTransform` in its N-input / M-output form. A single device-wide call reads from N input sequences and writes to M output sequences.

### [jitLto](./jitLto) [ ]
This sample does a simple saxpy multiply and add using nvrtc and nvJitLink with LTO (Link Time Optimization).

### [libcuxxMdspan](./libcuxxMdspan) [ ]
This sample demonstrates two mdspan-centric features of CCCL: DLPack <-> `cuda::std::mdspan` bridging and `cuda::shared_memory_mdspan` for multi-dimensional views of shared-memory tiles.

### [libcuxxRandom](./libcuxxRandom) [ ]
This sample demonstrates the random-number facilities added to libcu++ in CCCL (`<cuda/std/random>` and `<cuda/random>`).
